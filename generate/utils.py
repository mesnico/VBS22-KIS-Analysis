...
def get_team_values_df(data, team, team_logs):
        ...
        df['task_start'] = df['task'].apply(lambda x: runreader.get_task_from_taskname(x).started)
        df['time_best_video'] = (df['timestamp_best_video'] - df['task_start']) / 1000
        df['time_best_shot'] = (df['timestamp_best_shot'] - df['task_start']) / 1000
        df['time_correct_submission'] = df.apply(lambda x: runreader.get_correct_submission_times()[x['team']][x['task']] - runreader.get_task_from_taskname(x['task']).started, axis=1)
