...
def get_data_from_raw_files(args):
    ...
    if os.path.exists(resfile):
        ...
        runreader = competition_data['runreader']  # RunReader2023
        run = runreader.run  # original run file
        vbsRunID = run['id']['string']  # RunId of the official VBS 2023
        tasks = runreader.tasks  # it is used to check if a timestamp is inside a KIS task

        ## reading the raw files


        print(f"Reading file {raw_events}, about {round(os.stat(raw_events).st_size / (1024 * 1024))} MB ...")
        with open(raw_events, 'r') as f:
            events = json.load(f)
            for raw_event in events:
                ...
                assert not df.empty, 'Unrecognized session - team:'+raw_event['session']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--raw_events', default='../../data/2023/vbs23_events.json')
    parser.add_argument('--output_folder', default='../../data/2023/team_logs/from_dres')
    parser.add_argument('--audits_file', default='../../data/2023/vbs23_audits.jsonl')
    parser.add_argument('--run_file', default='../../data/2023/vbs23_run.json')
